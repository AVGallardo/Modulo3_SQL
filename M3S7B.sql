...
-- 2
SELECT a.folio, a.fecha, a.dias, a.valor_dia, 
       (SELECT c.nombre FROM clientes c WHERE c.rut = a.rut) AS NombreCliente,
       a.rut AS RutCliente
FROM arriendos a;

-- 3
SELECT c.rut, c.nombre, h.nombre AS herramienta_mas_arrendada
FROM clientes c
JOIN (
    SELECT a.rut, a.id_herramienta
    FROM arriendos a
    GROUP BY a.rut, a.id_herramienta
    ORDER BY COUNT(*) DESC
    LIMIT 1
) sub ON c.rut = sub.rut
JOIN herramientas h ON sub.id_herramienta = h.id_herramienta;

--4

SELECT c.rut, c.nombre, 
       CASE 
           WHEN sub.cantidad_arriendos BETWEEN 0 AND 1 THEN 'Bajo'
           WHEN sub.cantidad_arriendos BETWEEN 2 AND 3 THEN 'Medio'
           ELSE 'Alto'
       END AS clasificacion
FROM clientes c
LEFT JOIN (
    SELECT a.rut, COUNT(*) AS cantidad_arriendos
    FROM arriendos a
